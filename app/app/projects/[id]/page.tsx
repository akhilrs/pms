...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  return {
    title: project ? `${project.name} | Basecamp Clone` : 'Project Details | Basecamp Clone',
                ...
                {project.end_date && (
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <div className={`text-sm ${daysRemaining && daysRemaining < 0 ? 'text-red-500' : 'text-blue-500'}`}>
                ...
                <div className="flex flex-wrap gap-2">
                  {project.members && project.members.map((member) => (
                    <Avatar key={member.id} className="h-8 w-8">
                      ...
                      />
                      <AvatarFallback>
                        {getInitials(`${member.user?.first_name || ''} ${member.user?.last_name || ''}`.trim())}
                      </AvatarFallback>
                    </Avatar>
                  ))}
                </div>
