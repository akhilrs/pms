...
export async function createTeam(formData: {
  ...
  try {
    ...
    if (data) {
      console.log(`Adding creator ${session.user.id} as owner to team ${data.id}`);
      ...
      if (checkError) {
        console.error('Error checking for existing team membership:', checkError);
      }
      ...
      if (existingMember) {
        ...
        if (memberError) {
          console.error('Error adding team creator as member (using service role):', memberError);
        } else {
...
export async function updateTeam(
  ...
  try {
    ...
    if (!membership || !['admin', 'owner'].includes(membership.role)) {
      return { success: false, error: 'You do not have permission to update this team' };
    }
...
export async function addTeamMember(
  ...
  try {
    ...
    if (!membership || !['admin', 'owner'].includes(membership.role)) {
      return { success: false, error: 'You do not have permission to add members to this team' };
    }
...
export async function updateTeamMemberRole(
  ...
  try {
    ...
    if (!currentUserMembership || currentUserMembership.role !== 'owner') {
      return { success: false, error: 'Only team owners can change member roles' };
    }
    ...
    if (targetMembership.role === 'owner') {
      return { success: false, error: 'Cannot change the role of a team owner' };
    }
...
export async function removeTeamMember(
  ...
  try {
    ...
    if (!currentUserMembership || !['admin', 'owner'].includes(currentUserMembership.role)) {
      return { success: false, error: 'You do not have permission to remove members from this team' };
    }
    ...
    if (targetMembership.role === 'owner') {
      ...
      if (count === 1) {
        return { success: false, error: 'Cannot remove the last owner of the team' };
      }
    }
...
export async function assignTeamToProject(
  ...
  try {
    ...
    if (!project || project.owner_id !== session.user.id) {
      return { success: false, error: 'Only project owners can assign teams to projects' };
    }
    ...
    if (!teamMembership) {
      return { success: false, error: 'Team not found or you are not a member' };
    }
    ...
    if (existingAssignment) {
      return { success: false, error: 'Team is already assigned to this project' };
    }
...
export async function removeTeamFromProject(
  ...
  try {
    ...
    if (!project || project.owner_id !== session.user.id) {
      return { success: false, error: 'Only project owners can remove teams from projects' };
    }
